...
# Fix SeismoCore Explore Module 3.5 body-wave phase interaction without running npm.
# It injects a corrected interactive replacement into the built GitHub Pages docs/index.html.
# It also patches src/App.jsx text labels where simple replacements are safe.

mkdir -p docs

cat > docs/phase35-corrector.js <<'JS'
(function () {
  const PHASES = {
    P: {
      title: 'P',
...
# Inject runtime correction into built GitHub Pages HTML.
for name in ["docs/index.html"]:
    p = Path(name)
    if p.exists():
        txt = p.read_text(encoding="utf-8", errors="ignore")
        tag = '<script src="./phase35-corrector.js"></script>'
        if 'phase35-corrector.js' not in txt:
            txt = txt.replace('</body>', tag + '\n</body>') if '</body>' in txt else txt + '\n' + tag
            p.write_text(txt, encoding="utf-8")
            print(f"Injected {tag} into {name}")
        else:
            print(f"{name} already contains phase35-corrector.js")
    else:
        print(f"Skipped {name}: not found")

# Patch source text used during future Vite builds where possible.
p = Path('src/App.jsx')
if p.exists():
    s = p.read_text(encoding='utf-8', errors='ignore')
    replacements = {
        "S：剪力波，只能在固體中傳播，外地核為液態所以 S 波不能穿過外核。": "S：剪力波，只能在固體中傳播；外地核為液態，所以 S 波不能穿過外核。互動圖中 S 線必須留在固態地函內。",
        "顏色提示：穿越外核的路徑會偏金色；在 CMB 反射或沿 CMB 繞射的路徑，會貼近核幔邊界。": "顏色提示：K 代表外核中的 P 波；S、SS、ScS 不得進入液態外核；SKS/SKKS 的外核段是 K，不是 S。",
        "setPhase('S')} d=\"M148 314 C240 250, 376 250, 472 313\"": "setPhase('S')} d=\"M148 122 C225 330, 395 330, 472 122\"",
        "setPhase('PcP')} d=\"M154 122 C240 230, 281 298, 310 313 C339 298, 380 230, 466 122\"": "setPhase('PcP')} d=\"M154 122 C220 245, 275 300, 310 306 C345 300, 400 245, 466 122\"",
        "setPhase('PKP')} d=\"M150 122 C248 252, 282 260, 310 220 C338 260, 372 252, 470 122\"": "setPhase('PKP')} d=\"M150 122 C220 190, 265 265, 310 258 C355 265, 400 190, 470 122\"",
        "setPhase('PKIKP')} d=\"M150 122 C232 272, 288 255, 310 220 C332 185, 388 168, 470 122\"": "setPhase('PKIKP')} d=\"M150 122 C220 210, 280 235, 310 210 C340 185, 400 170, 470 122\"",
        "setPhase('Pdiff')} d=\"M145 122 C178 300, 442 300, 475 122\"": "setPhase('Pdiff')} d=\"M145 122 C215 220, 245 294, 278 304 C310 315, 342 304, 375 294 C405 220, 475 122\"",
    }
    changed = False
    ...
    if changed:
        ...
        print('Patched src/App.jsx source labels/paths where possible.')
    else:
        print('src/App.jsx found, but no exact old snippets matched; docs runtime fix is still installed.')
else:
    print('src/App.jsx not found; docs runtime fix only.')

print('Done. No npm install/build was run.')
PY
